...
const StackBarChart = ({
  ...
  const dataCount = mapData.filter(
    (x) => x.department === "SUNRISE" && x.Hold_Status === "OverDue"
  ).length;
  const dataCount1 = mapData.filter(
    (x) => x.department === "SUNRISE" && x.Hold_Status === "ON TIME"
  ).length;

  const dataSecondCount = mapData.filter(
    (x) => x.department === "BRAINWAVES" && x.Hold_Status === "OverDue"
  ).length;
  const dataSecondCount1 = mapData.filter(
    (x) => x.department === "BRAINWAVES" && x.Hold_Status === "ON TIME"
  ).length;

  const data = [
    { id: dataCount ? dataCount : "", ui: dataCount1 ? dataCount1 : "" },
    {
      id: dataSecondCount ? dataSecondCount : "",
      ui: dataSecondCount1 ? dataSecondCount1 : "",
    },
  ];
  ...
  const options = {
    ...
    onClick: (event, chart) => {
      if (chart.length > 0) {
        ...
        const overDeuDataClick = mapData.filter(
          (x) => x?.department === "SUNRISE" && x?.Hold_Status === "OverDue"
        );
        const ontimeData = mapData.filter(
          (x) => x?.department === "BRAINWAVES" && x?.Hold_Status === "OverDue"
        );
        const ontimeData2 = mapData.filter(
          (x) => x?.department === "BRAINWAVES" && x?.Hold_Status === "ON TIME"
        );

        if (clickedData === overDeuDataClick.length) {
          // const filteredOverdueData = mapData.filter(
          //   (x) => x?.department === "SUNRISE" && x?.Hold_Status === "OverDue"
          // );
          // const filteredOnTimeData = mapData.filter(
          //   (x) => x?.department === "SUNRISE" && x?.Hold_Status === "ON TIME"
          // );
          // console.log("filteredOverdueData",filteredOverdueData, filteredOnTimeData);
          setChartTable(
            mapData.filter(
              (x) => x.department === "SUNRISE" && x.Hold_Status === "OverDue"
            )
          ...
          const filteredOverdueData = mapData?.filter(
            (x) =>
              x?.department === "BRAINWAVES" && x?.Hold_Status === "OverDue"
          );

          setChartTable(filteredOverdueData);
          setItemData("BRAINWAVES");
        } else if (clickedData === ontimeData2.length) {
          const filteredOnTimeData = mapData?.filter(
            (x) =>
              x?.department === "BRAINWAVES" && x?.Hold_Status === "ON TIME"
          );
          setChartTable(filteredOnTimeData);
        }
        setActiveChartDefault();
      }
    },
